...
const Invoices = ({ T, reloadTimerSessions }) => {
  ...
  const changeStatus = async (id, status) => {
    try {
      ...
      if (status === 'payée') {
        // Re-fetch la facture depuis Supabase pour avoir les items complets
        const { data: invoice } = await supabase.from('invoices').select('*').eq('id', id).single()

        if (invoice?.items && Array.isArray(invoice.items)) {
          // Marquer les sessions comme payées (invoiced = true)
          ...
          // Marquer les daily_services inclus comme inactifs (is_active = false)
        ...
        // Recharger l'historique du timer (retirer les sessions payées)
        if (reloadTimerSessions) reloadTimerSessions()
        // Recharger les sessions/services dispo dans le formulaire de facture
