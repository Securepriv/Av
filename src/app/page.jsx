...
<div style={{display:'flex',flexDirection:'column',gap:6,width:'100%',...st}}>
    {label&&
<label style={{fontSize:12,color:T.textMuted,fontWeight:600}}>{label}</label>
}
    ...
    {error&&
<div style={{fontSize:11,color:T.danger,marginTop:2}}>{error}</div>
}
  </div>
)
...
return <span style={{display:'inline-flex',alignItems:'center',gap:4,background:s.bg,color:s.color,fontSize:11,fontWeight:600,padding:'3px 10px',borderRadius:20}}><span style={{width:5,height:5,borderRadius:'50%',background:s.color,flexShrink:0}}/>{status}
</span>
}
...
return(
 ...
 {<code>@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&amp;display=swap');*{box-sizing:border-box;margin:0;padding:0}input,button{f ont-family:inherit;outline:none} </ code > } <div className= "fade-up " style={{width:'100%',maxWidth:420}} >
...
{success & & <div style={{marginTop:14,padding:'10px 14px',background:'#DCFCE7',border:'1px solid #15803D30',borderRadius:8,fontSize:13,color:'#15803D'}} >{success} </div >}
 <button onClick={handle} disabled={loading} style={{marginTop:20,width:'100%',padding:'11px',borderRadius:8,border:'none',background:'#1A1916',color:'#FFFFFF',fontWeight:700,fontSi ze:14,cursor:loading?'not-allowed':'pointer',fontFamily:'inherit',opacity:loading?.7:1}} >
...
{[
{label:'Encaissé ce mois',value:
`${(stats?.billed||0).toLocaleString('fr')} €`
,sub:'Factures payées uniquement',color:T.accent},
{label:'En attente paiement',value:
`${(stats?.pending||0).toLocaleString('fr')} €`
,sub:
`${stats?.pendingCount||0} facture(s) envoyée(s)`
,color:T.warning},
{label:'Heures / semaine',value:
`${stats?.weekHours||0}h`
,sub:'7 derniers jours',color:T.info},
...
ℹ️ Factures avec statut 
<strong>Envoyée</strong> ou 
<strong>En retard</strong> — transmises au client mais pas encore réglées. Les brouillons et les payées ne s'affichent pas ici.
</div>
{pendingInvoices.length===0
?<Empty icon="✓" text="Aucune facture en attente !" sub="Toutes tes factures sont réglées." T={T}/>
:pendingInvoices.map((inv,i)=>(
<div key={inv.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'11px 0',borderBottom:i<pendingInvoices.length-1?
`1px solid ${T.border}`
:'none'}}>
...
{(stats?.pendingCount||0) >0 & &(
 <Card T={T} style={{background:T.warningBg,borderColor:
`${T.warning}30`
}}>
...
const[selectedClient,setSelectedClient]=useState(null) // client dont on affiche le détail
const[clientDetail,setClientDetail]=useState(null) // sessions + factures du client
...
const payload={user_id:user.id,first_name:form.firstName,last_name:form.lastName,name:`${form.firstName} ${form.lastName}`.trim(),email:form.email,phone:form.phone,company:c.company||form.company,vat_id:form.vatId,color:form.color,rate:Number(form.rate)||0,custom_fields:form.customFields.filter(f=>f.label.trim())}
       ...
       < >
        {/* Stats du client */}
         <div className= "stats-grid " style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}} >
          {[
            {label:'Heures non facturées',value:`${totalUnpaidH.toFixed(1)}h`,sub:`${((totalUnpaidH)*(selectedClient.rate||0)).toFixed(0)} € à facturer`,color:T.warn ing},
            {label:'Heures facturées',value:`${totalPaidH.toFixed(1)}h`,sub:'Travail payé',color:T.success},
            {label:'Chiffre d\'affaires',value:`${totalRevenue.to LocaleString('fr')} €`,sub:'Factures payées',color:T.accent},
       ...
       <div className= "grid-2 " style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}} >
         ...
         <div >
           ...
           <div style={{display:'flex',gap:8,flexWrap:'wrap'}} >
            {CLIENT_COLORS.map(c= >(
               <div key={c} onClick={()= >setForm({...form,color:c})} style={{width:26,height:26,borderRadius:'50%',background:c,cursor:'pointer',border:form.color===c?`3px solid ${T.text}`:'3px solid transparent',transit ion:'all .15s'}}/>
            ))}
           </div >
         </div >
       </div >
...
:paidSessions.map((s,i)= >(
 <div key={s.id} style={{display:'flex',justifyContent:'space-between',padding:'9px 0',borderBottom:i <paidSessions.length-1?
`1px solid ${T.border}`
:'none'}}>
...
:paidInvoices.map((inv,i)= >(
 <div key={inv.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:i <paidInvoices.length-1?
`1px solid ${T.border}`
:'none'}}>
...
<div style={{borderTop:
`2px solid ${T.border}`
,paddingTop:12,marginTop:8,display:'flex',justifyContent:'space-between'}}>
<span style={{fontSize:13,fontWeight:600,color:T.text}}>Total encaissé</span><span className="mono" style={{fontSize:15,fontWeight:700,color:T.success}}>{totalRev.toLocaleString('fr')} €</span>
</div>
)}
</Card >
</div>
...
{clients.map(c= >(
 ...
 <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:14}} >
 <div style={{width:44,height:44,borderRadius:'50%',background:c.color+'20',border:
`2px solid ${c.color}50`
,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,fontSize:16,color:c.color,flexShrink:0}}>
{(c.first_name||c.name||'?')[0].toUpperCase()}
</div >
      ...
      {/* Sélection client */}
       <div style={{marginBottom:12}} >
         ...
         <select value={clientId} onChange={e= >setClientId(e.target.value)} disabled={running}
          style={{width:'100%',background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'9px 12px',color:T.te xt,fontSize:13,cursor:running?'not-allowed':'pointer',opacity:running?.7:1}} >
         ...
         <div style={{display:'flex',gap:8}} >
          ...
          {!paused?(
             <button onClick={pause} style={{
              ...
              background:T.warningBg,color:T.warning,fontWeig ht:700,fontSize:13,
             ...
             <button onClick={resume} style={{
              ...
              background:T.accentLight,color:T.accent,fontWei ght:700,fontSize:13,
              cursor:'pointer',fontFamily:'inherit',transition:'all .15s',
              display:'flex',alignItems:'center',justifyContent:'center',gap:7
             }}
              onMouseEnter={e= >e.currentTarget.style.opacity='.8'}
          ...
          {/* Arrêter */}
           <button onClick={handleStop} disabled={saving} style={{
            ...
            display:'flex',alignItems:'center',jus tifyContent:'center',gap:7,opacity:saving?.7:1
          }}
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  @media print{
    ...
    html{-webkit-print-color-adjust:exact;print-color-adjust:ex act}
  }
  ...
  .parties{display:grid;grid-template-columns:1fr 1f r;gap:40px;margin-bottom:36px}
  .party-label{font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#A8A5A0;margin-bottom:8px}
  .party-name{font-size:1 5px;font-weight:700;color:#1A1916;margin-bottom:4px}
  .party-detail{font-size:12px;color:#6B6963;line-height:1.7}
  .dates{display:grid;grid-template-columns:1fr 1fr;gap:16px;back ground:#F8F7F4;border-radius:10px;padding:16px 18px;margin-bottom:30px}
  .date-item label{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#A8A5A 0;display:block;margin-bottom:4px}
  .date-item span{font-size:14px;font-weight:600;color:#1A1916}
  table{width:100%;border-collapse:collapse;margin-bottom:22px}
  thead tr{backgr ound:#1A1916;color:#FFFFFF}
  thead th{padding:11px 14px;text-align:left;font-size:11px;font-weight:600;letter-spacing:.06em;text-transform:uppercase}
  thead th:last-child{text-al ign:right}
  tbody tr{border-bottom:1px solid #E0DED8}
  tbody tr:last-child{border-bottom:none}
  tbody td{padding:12px 14px;font-size:13px;color:#1A1916}
  tbody td:last-child{te xt-align:right;font-weight:600;font-family:monospace}
  .totals{display:flex;justify-content:flex-end;margin-bottom:36px}
  .totals-box{width:280px}
  .total-row{display:flex;justi fy-content:space-between;padding:8px 0;font-size:13px;color:#6B6963;border-bottom:1px solid #E0DED8}
  ...
  .payment{background:#F8F7F4;border-radiu s:10px;padding:18px 20px;margin-bottom:30px}
  ...
  .footer{border-top:1px solid #E0DED8;padding-top:18px;text-align:center;font-size:11px;color:#A8A5A0;line-he ight:1.8}
</style>
</head>
<body>
  <div class="header">
...
const[visibleFields,setVisibleFields]=useState({invoice_date:true})
const notify=(msg,type='ok')=>{setToast({msg,type});setTimeout(()=>setToast(null),3000)}
const load=useCallback(async()=>{
const {data:{user}} = await supabase.auth.getUser()
...
const tvaAmount = form.tvaRate ? htTotal * (Number(form.tvaRate) / 100) : 0
...
if(invoiceMode==='hours'){
  ...
  items=freeIt ems.filter(i= >i.service & &Number(i.rate) >0).map(i= >({
    service:i.service,
    qty:Number(i.qty||1),
...
const payload={
  ...
  amount:grandTotal,
  amount_ht: htTotal,
  tva_rate: form.tvaRate ? Number(form.tvaRate) : 0,
  ...
  free_clien t_email: invoiceMode==='free'?form.freeClientEmail:null,
}

const{error}=await supabase.from('invoices').insert(payload)
if(error){notify(error.message,'error');setSaving(false);re turn}
...
setShowForm(false);setSelectedSessions([]);setFreeItems([{serv ice:'',qty:1,rate:0,unit:'forfait'}])
...
const fields=[
  ...
  {key:'av_email',  label:`Ton email : ${avData?.email||'No n renseigné'}`,      warn:!avData?.email},
  ...
  {key:'client_name',   la bel:`Client : ${clientName}`},
  {key:'client_company',label:`Société : ${pdfClient?.company||'—'}`,            warn:false},
  {key:'client_email',  label:`Email client : ${clientE mail||'—'}`,              warn:false},
  {key:'client_phone',  label:`Tél client : ${pdfClient?.phone||'—'}`,           warn:false},
  {key:'client_vat',    label:`TVA client : ${p dfClient?.vat_id||'—'}`,          warn:false},
]
          ...
          {fields.map(f= >{
            ...
            return(
               <div key={f.key} onClick={()= >setVisibleFields(v= >({...v,[f.key]:!visible}))}
                style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'7px 10px',borderRadius:8,background:visible?T.accent Light:T.surfaceHigh,border:`1px solid ${visible?T.accent+'40':T.border}`,cursor:'pointer',transition:'all .15s'}} >
           ...
           <div style={{display:'flex',justifyContent:'flex-end'}} >
             <div style={{width:200,borderTop:'2px solid #1A1916',paddingTop:10}} >
               <div style={{display:'flex',justifyContent:'space-between',fontSize:13,color:'#6B6963',marginBottom:4}}>
                 <span >Total HT</span>
                 <span style={{fontFamily:'monospace'}} >{Number(pdfInvoice.amount_ht||pdfInvoice.amount||0).toLocaleString('fr-FR',{minimumFractionDigits:2})} € </span >
               </div>
               ${pdfInvoice.tva_rate > 0 ? `
               <div style={{display:'flex',justifyContent:'space-between',fontSize:13,color:'#6B6963',marginBottom:4}}>
                 <span >TVA (${pdfInvoice.tva_rate}%)</span>
                 <span style={{fontFamily:'monospace'}} >${(Number(pdfInvoice.amount_ht||pdfInvoice.amount||0) * (pdfInvoice.tva_rate/100)).toLocaleString('fr-FR',{minimumFractionDigits:2})} € </span >
               </div>` : `
               <div style={{display:'flex',justifyContent:'space-between',fontSize:13,color:'#6B6963',marginBottom:4}}>
                 <span >TVA</span>
  ...
  {showForm & &(
     <Card accent T={T} style={{marginBottom:20}} className= "fade-up " >
       ...
       <div style={{display:'flex',gap:8,marginBottom:20}} >
        {[{id:'hours',label:'Par heures (client existant)',icon:'◷'},{id:'free',label:'Libre (forfait, mission ponctuelle)',icon:'◈'}].map(m= >(
           <button key={m.id} onClick={()= >setInvoiceMode(m.id)} style={{
            ...
            background:invoiceMod e===m.id?T.accentLight:'transparent',
            color:invoiceMode===m.id?T.accent:T.textMuted,fontSize:13,fontWeight:invoiceMode===m.id?600:400,
            cursor:'pointer',font Family:'inherit',transition:'all .15s',textAlign:'left',
          }} >
      ...
      {/* Infos communes */}
        ...
        {invoiceMode==='hours'?(
           <div >
             ...
             <select value={form.clientId} onChange={e= >setForm({...form,clientId:e.target.value})}
              style={{width:'100%',background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'9px 12px',color:T.te xt,fontSize:13}} >
              {clients.map(c= > <option key={c.id} value={c.id} >{c.name} — {c.rate}€/h </option >)}
             </select >
           </div >
        ):(
       ...
       {/* TVA Field */}
       <div style={{marginBottom:16}}>
         <Input label="TVA (%)" type="number" value={form.tvaRate} onChange={v=>setForm({...form,tvaRate:v})} placeholder="21" suffix="%" T={T}/>
         <div style={{fontSize:11,color:T.textDim,marginTop:4}}>Laissez vide ou 0 si non assujetti (N/A).</div>
       </div>
      ...
      {invoiceMode==='hours' & &(
         <div style={{marginBottom:16}} >
                ...
                {sessions.map((s,i)= >{
                  ...
                  return(
                    <div key={s.id} onClick={()= >toggleSession(s.id)} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',borderBottom:i <sessions.length-1?`1px solid ${T.border}`:'none',cursor:'pointer',background:checked?T.accentLight:'transparent',transition:'background .12s'}} >
                       <div style={{width:18,height:18,borderRadius:5,border:`1.5px solid ${checked?T.accent:T.border}`,background:checked?T.accent:'transparent',display:'flex',alignItems:'center',justif yContent:'center',flexShrink:0,transition:'all .15s'}} >
          ...
          {selectedSessions.length >0 & &(
             <div style={{marginTop:10,padding:'10px 14px',background:T.accentLight,border:`1px solid ${T.accent}30`,borderRadius:8,display:'flex',justifyContent:'space-between',alignItems:'cen ter'}} >
      ...
      {/* TOTAL + ACTIONS */}
       <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',borderTop:`1px solid ${T.border}`,paddingTop:16}} >
         <div >
           ...
           <div style={{fontSize:11,color:T.textMuted,marginTop:3}}>HT: {htTotal.toFixed(2)} € {form.tvaRate ? `· TVA (${form.tvaRate}%): ${tvaAmount.toFixed(2)} €` : ''}</div>
...
<body>
  <div class="header">
    ...
    <div>
      <h1>DEVIS</h1>
      <div class="ref">${quote.ref||'DEV-001'}</div>
    </div>
  </div>
  ...
  <div class="info-box">
    ...
    <div class="info-item">
      ...
      <span>${quote.valid_days||30} jours</span>
    </div>
  </div>
  ...
  <div class="footer">
    ${settings?.full_name||''} ${settings?.siret?`· SIRET ${settings.siret}`:''}<br/>
    Merci pour votre confiance.
  </div>
</body>
</html>
...
const[saving,setSaving]=useState(false)
const[toast,setToast]=useState(null)
// Formulaire libre — pas besoin de client existant
const emptyForm={
prospectName:'',prospectEmail:'',prospectCompany:'',
items:[{service:'',qty:1,rate:0}],
note:'',validDays:30,tvaRate:''
}
const[form,setForm]=useState(emptyForm)
const notify=(msg,type='ok')=>{setToast({msg,type});setTimeout(()=>setToast(null),3000)}
const load=useCallback(async()=>{
...
<Btn onClick={()=>setShowForm(!showForm)} T={T}>+ Nouveau devis</Btn>
  ...
  {/* Info explication */}
   <div className= "fade-up s1 " style={{padding:'10px 16px',background:T.infoBg,borderRadius:10,fontSize:13,color:T.info,marginBottom:20,lineHeight:1.6}} >
    ℹ️ Un devis est  <strong >libre </strong > — tu n'as pas besoin que le prospect soit dans ta liste de clients. Tu remplis son nom, email et les prestations demandées. Une fois accepté, tu crées la facture depuis la sectio n Factures.
   </div >
     ...
     <Card accent T={T} style={{marginBottom:20}} className= "fade-up " >
         ...
         <div style={{background:T.surfaceHigh,borderRadius:10,padding:'4px 0',marginBottom:10}} >
          {/* En-tête colonnes */}
           <div style={{display:'grid',gridTemplateColumns:'2fr 100px 120px 120px 36px',gap:8,padding:'8px 14px',borderBottom:`1px solid ${T.border}`}} >
            {['Prestation','Qté / h','Tarif €','Total',''].map(h= >(
               <div key={h} style={{fontSize:11,color:T.textDim,fontWeight:600,textTransform:'uppercase',letterSpacing:'.05em'}} >{h} </div >
            ))}
           </div >
          {form.items.map((item,i)= >{
            ...
            return(
               <div key={i} style={{display:'grid',gridTemplateColumns:'2fr 100px 120px 120px 36px',gap:8,padding:'8px 14px',borderBottom:i <form.items.length-1?`1px solid ${T.border}`:'none',alignItems:'center'}} >
                 <select value={item.service} onChange={e= >updateItem(i,'service',e.target.value)}
                  style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:7,padding:'7px 10px',color:item.service?T.text:T .textDim,fontSize:13}} >
                   <option value= " " >Choisir... </option >
                  {VA_SERVICES.map(s= > <option key={s} >{s} </option >)}
                   <option value= "Autre " >Autre / personnalisé </option >
                 </select >
                 <input type= "number " placeholder= "1 " value={item.qty||''} onChange={e= >updateItem(i,'qty',e.target.value)}
                  style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:7,padding:'7px 10px',color:T.text,fontSize:13,textAl ign:'center'}}/>
                 <input type= "number " placeholder= "0 € " value={item.rate||''} onChange={e= >updateItem(i,'rate',e.target.value)}
                  style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:7,padding:'7px 10px',color:T.text,fontSize:13}}/>
                 <div className= "mono " style={{fontSize:13,fontWeight:600,color:lineTotal >0?T.accent:T.textDim,paddingLeft:4}} >
                  {lineTotal >0?`${lineTotal.toFixed(2)} €`:'—'}
                 </div >
                 <button onClick={()= >removeItem(i)} disabled={form.items.length===1}
                  style={{background:'none',border:'none',color:form.items.length===1?T.textDim:T.danger,cursor:form.items.length== =1?'not-allowed':'pointer',fontSize:14,padding:4}} >✕ </button >
               </div >
            )
          })}
         </div >
      ...
      {/* Note */}
       <div style={{marginBottom:16}} >
         ...
         <textarea value={form.note} onChange={e= >setForm({...form,note:e.target.value})} placeholder= "Conditions particulières, délai de réalisation, etc. " rows={3}
          style={{width:'100%',background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'10px 13px',color:T.text,fontSize:13,resize:'vertical',font Family:'inherit'}}/>
       </div >
      ...
      {/* Total + actions */}
       <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',borderTop:`1px solid ${T.border}`,paddingTop:16}} >
         <div >
           ...
           <div style={{fontSize:12,color:T.textMuted,marginTop:3}}>HT: {htTotal.toFixed(2)} € {form.tvaRate ? `· TVA (${form.tvaRate}%): ${tvaAmount.toFixed(2)} €` : ''}</div>
               ...
               <div style={{display:'flex',gap:16,alignItems:'center'}} >
                 <div >
                  ...
                  {q.prospect_company & & <div style={{fontSize:12,color:T.textMuted}} >{q.prospect_company} </div >}
                   <div style={{fontSize:11,color:T.textDim,marginTop:4}} >{new Date(q.created_at).toLocaleDateString('fr')} · {Array.isArray(q.items)?q.items.length:Array.isArray(q.services)?q.services.length:0} prestation(s) </div >
                 </div >
               </div >
      ...
      {/* THEME */}
       <Card T={T} className= "fade-up s2 " >
         ...
         <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}} >
          {['light','dark'].map(t= >(
             <div key={t} onClick={()= >setTheme(t)} style={{padding:'14px',borderRadius:10,border:`2px solid ${theme===t?T.accent:T.border}`,cursor:'pointer',textAlign:'center',background:theme===t?T.accentLight:T.surf aceHigh,transition:'all .15s'}} >
...
<div style={{width:16,height:16,border:`2px solid ${T.border}`,borderTopColor:T.accent,borderRadius:'50%',animation:'spin .6s linear infinite'}}/>
{<code>@keyframes spin{to{transform:rotate(360deg)}}</code>}
</div>
)
...
return(
<>
{makeCSS(T)}<div className="app-layout" style={{display:'flex',minHeight:'100vh',background:T.bg}}>
     ...
     <div style={{
      ...
      {/* Chrono */}
       <div className= "mono " style={{fontSize:18,fontWeight:400,
        ...
        animation:timerPaused?'none':'blink 2.5s ease i nfinite'
      }} >
       ...
       <div style={{display:'flex',gap:6}} >
        {!timerPaused?(
         ...
         <button onClick={()= >setPage('timer')} style={{
          ...
          fontSize:12,fontW eight:500,cursor:'pointer',fontFamily:'inherit',transition:'all .15s'
        }}
