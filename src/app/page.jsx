    ...
    >
      ...
      <div className="sidebar-footer" style={{ marginTop: 'auto', paddingTop: 20, borderTop: `1px solid ${T.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          <div style={{ width: 36, height: 36, background: T.surfaceHigh, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14, color: T.text, border: `2px solid ${T.accent}30` }}>
        ...
        <button
          onClick={onSignOut}
          style={{ width: '100%', background: 'transparent', border: '1px solid #44403C', borderRadius: 10, padding: '10px', fontSize: 13, color: '#A8A29E', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontWeight: 500 }}
        >
...
const PaidWork = ({ T }) => {
  ...
  const openClient = async (c) => {
    ...
    setLoadingDetail(true)
    try {
      const sesRes = await supabase.from('sessions').select('').eq('client_id', c.id).eq('invoiced', true).order('date', { ascending: false })
      const invRes = await supabase.from('invoices').select('').eq('client_id', c.id).eq('status', 'payée').order('created_at', { ascending: false })
    ...
    return (
      <div className="main-content" style={{ padding: '32px 40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 14, color: T.success }}>{totalH.toFixed(1)}h travaillées · {totalRev.toLocaleString('fr')} € encaissés</div>
            </div>
          </div>
        </div>
        ...
        {loadingDetail ? (
          ...
          <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <Card T={T}>
              <div style={{ fontWeight: 700, fontSize: 16, color: T.text, marginBottom: 16 }}>Sessions payées</div>
              {paidSessions.length === 0 ? (
                <Empty icon="⏱️" text="Aucune session payée" sub="" T={T} />
              ) : (
                paidSessions.map((s, i) => (
                  <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: i < paidSessions.length - 1 ? `1px solid ${T.border}` : 'none' }}>
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: 10, background: T.successBg, color: T.success, padding: '2px 8px', borderRadius: 6, fontWeight: 600, marginTop: 4, textAlign: 'center', display: 'inline-block' }}>Payée</div>
                    </div>
                  </div>
                ))
              )}
    ...
    <div className="main-content" style={{ padding: '32px 40px' }}>
      ...
      {!showDailyForm ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          {/* Services journaliers — toujours affichés sous le timer */}
          <Card T={T} className="fade-up">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div style={{ fontWeight: 700, fontSize: 16, color: T.text }}>📋 Services journaliers enregistrés</div>
              <Btn small onClick={() => setShowDailyForm(true)} T={T} icon="➕">Ajouter un service</Btn>
            </div>
            {dailyServices.length === 0 ? (
              <Empty icon="📋" text="Aucun service enregistré" sub="Ajoute des services récurrents ou ponctuels via le bouton ci-dessus." T={T} />
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
                {dailyServices.map((ds) => {
                  const qty = ds.qty || ds.days_per_month || 1
                  return (
                    <div key={ds.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', background: T.surfaceHigh, borderRadius: 12, border: `1px solid ${T.border}` }}>
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <div style={{ fontWeight: 600, fontSize: 14, color: T.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ds.service}</div>
                        <div style={{ fontSize: 12, color: T.textMuted, marginTop: 3 }}>
                          <span style={{ background: T.accentLight, color: T.accent, padding: '1px 7px', borderRadius: 6, fontWeight: 600, fontSize: 11 }}>{ds.clients?.name}</span>
                          <span style={{ marginLeft: 8 }}>Qté: {qty} × {Number(ds.rate)}€</span>
                        </div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 12, flexShrink: 0 }}>
                        <span className="mono" style={{ fontSize: 15, fontWeight: 700, color: T.accent }}>{(Number(qty) * Number(ds.rate)).toLocaleString('fr')} €</span>
                        <button onClick={() => deleteDailyService(ds.id)} style={{ background: T.dangerBg, border: `1px solid ${T.danger}30`, borderRadius: 8, color: T.danger, cursor: 'pointer', padding: '5px 10px', fontSize: 13, fontFamily: 'inherit', fontWeight: 600 }}>✕</button>
                      </div>
                    </div>
                  )
                })}
            ...
            {dailyServices.length > 0 && (
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${T.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, color: T.textMuted }}>Total services : <strong style={{ color: T.accent }} className="mono">{dailyServices.reduce((a, ds) => a + (Number(ds.qty || ds.days_per_month || 1) * Number(ds.rate)), 0).toLocaleString('fr')} €</strong></span>
                <span style={{ fontSize: 12, color: T.textDim }}>💡 Ces services peuvent être ajoutés lors de la création d'une facture</span>
              </div>
            )}
          </Card>
        </div>
          ...
          <Card T={T}>
            ...
            {dailyServices.length === 0 ? (
              ...
              dailyServices.map((ds, i) => {
                const qty = ds.qty || ds.days_per_month || 1
                return (
                  <div key={ds.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 16, background: T.surfaceHigh, borderRadius: 10, marginBottom: i < dailyServices.length - 1 ? 12 : 0 }}>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 14, color: T.text }}>{ds.service}</div>
                      <div style={{ fontSize: 12, color: T.textMuted, marginTop: 3 }}>
                        {ds.clients?.name} · Qté: {qty} × {Number(ds.rate)}€
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <span className="mono" style={{ fontSize: 15, fontWeight: 600, color: T.accent }}>
                        {(Number(qty) * Number(ds.rate)).toLocaleString('fr')} €
                      </span>
            ...
            {dailyServices.length > 0 && (
              <div style={{ marginTop: 20, paddingTop: 20, borderTop: `1px solid ${T.border}` }}>
                <div style={{ fontSize: 13, color: T.textMuted, marginBottom: 12 }}>💡 Tu peux ajouter automatiquement ces services lors de la création d'une facture pour ce client.</div>
                <Btn full variant="success" onClick={() => (window.location.href = '/invoices')} T={T}>Créer facture →</Btn>
              </div>
            )}
          </Card>
        </div>
...
const Invoices = ({ T }) => {
  ...
  const loadClientData = useCallback(async (clientId) => {
    ...
    try {
      const [sesRes, dsRes] = await Promise.all([
        ...
        supabase.from('daily_services').select('*').eq('client_id', clientId)
      ])
  ...
  const dsAmount = checkedDailyServices.reduce((a, ds) => a + (Number(ds.qty || ds.days_per_month || 1) * Number(ds.rate)), 0)
    ...
    try {
      ...
      if (invoiceMode === 'hours') {
        ...
        const dsItems = checkedDailyServices.map((ds) => ({
          service: ds.service, qty: Number(ds.qty || ds.days_per_month || 1), rate: Number(ds.rate), unit: 'unité',
        }))
      ...
      setShowForm(false); setSelectedSessions([]); setSelectedDailyServices([]); setFreeItems([{ service: '', qty: 1, rate: 0, unit: 'forfait' }])
    ...
    try {
      ...
      if (status === 'payée') {
        ...
        if (invoice?.items) {
          ...
          if (sessionIds.length > 0) await supabase.from('sessions').update({ invoiced: true }).in('id', sessionIds)
        }
        if (form.clientId) loadClientData(form.clientId)
      }
                ...
                <div style={{ marginTop: 24 }}>
                  ...
                  <div style={{ border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden' }}>
                    {clientDailyServices.map((ds, i) => {
                      ...
                      const dsQty = Number(ds.qty || ds.days_per_month || 1)
                      const amount = dsQty * Number(ds.rate)
                      return (
...
export default function App() {
  ...
  const reloadTimerSessions = useCallback(() => {
    supabase.from('sessions').select('*,clients(name,color)').order('date', { ascending: false }).limit(20).then((res) => setTimerSessions(res.data || []))
  }, [])
  ...
  const pages = { dashboard: <Dashboard setPage={setPage} user={user} T={T} />, clients: <Clients T={T} setPage={setPage} />, paidwork: <PaidWork T={T} />, timer: <Timer T={T} timerState={timerState} timerActions={timerActions} />, invoices: <Invoices T={T} />, quotes: <Quotes T={T} />, settings: <Settings user={user} T={T} theme={theme} setTheme={setTheme} onSignOut={signOut} /> }
