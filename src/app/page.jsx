...
@media (max-width: 1024px) {
  ...
  .sidebar-nav div span:last-child {
    display: none;
  }
...
@media (max-width: 768px) {
  ...
  .sidebar-nav div span:first-child {
...
const Input = ({ label, value, onChange, type = 'text', placeholder, style: st, error, T, readOnly, suffix, multiline }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, ...st }}>
      ...
      {multiline ? (
        ...
        <input
          ...
          style={{
            ...
            border: `1px solid ${error ? T.danger : T.border}`,
            borderRadius: 10,
            padding: `11px 14px ${suffix ? '11px 40px' : '11px 14px'}`,
            color: T.text,
            fontSize: 14,
            width: '100%',
            transition: 'border-color 0.2s, box-shadow 0.2s',
    ...
    {error && (
      <span style={{ fontSize: 11, color: T.danger, fontWeight: 500 }}>
        {error}
      </span>
    )}
  </div>
...
const Empty = ({ icon, text, sub, T }) => (
  ...
  >
    ...
    {sub && (
      <div style={{ fontSize: 13, color: T.textMuted, maxWidth: 300 }}>
        {sub}
      </div>
    )}
  </div>
  ...
  msg ? (
    <div
      style={{
        ...
        zIndex: 9999,
        padding: '14px 20px',
        background: type === 'error' ? T.dangerBg : T.surface,
        border: `1px solid ${type === 'error' ? T.danger + '50' : T.border}`,
        borderRadius: 12,
        fontSize: 14,
        color: type === 'error' ? T.danger : T.text,
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {mode === 'login'
              ? 'Connecte-toi à ton espace VABilling.'
        ...
        {NAV.map((n) => (
          <div
            ...
            style={{
              ...
              color: page === n.id ? T.accent : T.sidebarMuted,
...
const Dashboard = ({ setPage, user, T }) => {
  ...
  return (
    <div className="main-content" style={{ padding: '32px 40px' }}>
      ...
      <div
        className="fade-up s1 stats-grid"
        style={{
          display: 'grid',
        ...
        {[
          {
            ...
            sub: 'Factures payées uniquement',
            color: T.accent,
            icon: '💰',
          },
          {
            ...
            sub: `${stats?.pendingCount || 0} facture(s)`,
            color: T.warning,
            icon: '⏳',
          },
          {
          ...
          <Card key={s.label} T={T} style={{ position: 'relative', overflow: 'hidden' }}>
            ...
            <div className="mono" style={{ fontSize: 28, fontWeight: 600, color: s.color, lineHeight: 1 }}>
              {s.value}
            </div>
      ...
      >
        <Card T={T}>
          ...
          >
            ...
            <Btn small variant="ghost" onClick={() => setPage('invoices')} T={T}>
              Voir tout →
            </Btn>
          </div>
          ...
          >
            <span>ℹ️</span>
            <span>
              Factures avec statut <strong>Envoyée</strong> ou <strong>En retard</strong>
            </span>
          </div>
            ...
            pendingInvoices.map((inv, i) => (
              <div
                ...
                style={{
                  ...
                  alignItems: 'center',
                  padding: '14px 0',
                  borderBottom: i < pendingInvoices.length - 1 ? `1px solid ${T.border}` : 'none',
                }}
              >
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: T.text }}>{inv.invoice_number}</div>
                  <div style={{ fontSize: 12, color: T.textMuted, marginTop: 2 }}>
                    {inv.clients?.name} · Échéance : {inv.due_date || '—'}
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  <span className="mono" style={{ fontSize: 15, fontWeight: 600, color: T.text }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <Card T={T}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 16, color: T.text }}>
              Actions rapides
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Btn full onClick={() => setPage('invoices')} T={T} icon="📄">
                Nouvelle facture
              </Btn>
              <Btn full variant="ghost" onClick={() => setPage('quotes')} T={T} icon="📋">
                Nouveau devis
              </Btn>
              <Btn full variant="ghost" onClick={() => setPage('timer')} T={T} icon="▶️">
                Démarrer timer
              </Btn>
              <Btn full variant="ghost" onClick={() => setPage('clients')} T={T} icon="👥">
                Ajouter client
              </Btn>
            </div>
          </Card>
          ...
          {stats?.pendingCount > 0 && (
            ...
            >
              <div style={{ fontSize: 13, color: T.warning, fontWeight: 700, marginBottom: 8 }}>
                ⚠️ Rappel paiement
              </div>
              <div style={{ fontSize: 13, color: T.textMuted, lineHeight: 1.6 }}>
                {stats.pendingCount} facture(s) non réglée(s) ·{' '}
                <strong style={{ color: T.text }}>
                  {(stats.pending || 0).toLocaleString('fr')} € en attente
                </strong>
              </div>
            </Card>
          )}
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
        <button
          ...
          style={{
            padding: '12px 24px',
            borderRadius: 12,
            border: `1px solid ${!showDailyForm ? T.accent : T.border}`,
            background: !showDailyForm ? T.accentLight : 'transparent',
            color: !showDailyForm ? T.accent : T.textMuted,
            fontWeight: 600,
            cursor: 'pointer',
            fontFamily: 'inherit',
            fontSize: 14,
      ...
      {!showDailyForm ? (
        ...
        >
          <Card accent={isActive} T={T} style={{ position: 'relative', overflow: 'hidden' }}>
            {running && (
              ...
              >
                <div
                  style={{
                    height: '100%',
                    background: paused ? T.warning : T.accent,
                    width: `${Math.min((seconds % 3600) / 3600 * 100, 100)}%`,
                  ...
                  >
                    <div
                      style={{
                        ...
                        animation: isActive ? 'blink 0.8s ease infinite' : 'none',
                      }}
                    />
                    <span
                      style={{
                        fontSize: 13,
                        color: paused ? T.warning : T.danger,
                        fontWeight: 600,
                      }}
                    >
                      {paused ? 'En pause — ' : ''}
            ...
            {!running ? (
              <Btn full onClick={() => start()} disabled={!clientId} T={T} icon="▶️">
                Démarrer
              </Btn>
            ) : (
            ...
            {running && (
              ...
              >
                ...
                <span className="mono" style={{ color: T.accent, fontWeight: 600 }}>
                  {(seconds / 3600).toFixed(2)}h · {((seconds / 3600) * (client?.rate || 0)).toFixed(2)} €
                </span>
              </div>
            )}
          </Card>
          ...
          <Card T={T}>
            ...
            >
              ...
              <div className="mono" style={{ fontSize: 14, color: T.accent, fontWeight: 600 }}>
                {totalH}h total
              </div>
            </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div
                      style={{
                        ...
                        height: 8,
                        borderRadius: '50%',
                        background: s.clients?.color || T.accent,
                        flexShrink: 0,
                      }}
                    />
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>
                        {s.clients?.name} · {s.date}
                      </div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span className="mono" style={{ fontSize: 14, color: T.accent, fontWeight: 600 }}>
                      {s.duration}h
                    </span>
                    {s.invoiced && (
                      <span
        ...
        >
          <Card accent T={T}>
            <div style={{ fontWeight: 700, fontSize: 18, color: T.text, marginBottom: 20 }}>
              📋 Ajouter un service quotidien
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label
                ...
                <select
                  value={dailyForm.clientId}
                  onChange={(e) => setDailyForm({ ...dailyForm, clientId: e.target.value })}
                  style={{
                    width: '100%',
                    background: T.surfaceHigh,
                    border: `1px solid ${T.border}`,
              ...
              dailyServices.map((ds, i) => (
                <div
                  ...
                  style={{
                    ...
                    background: T.surfaceHigh,
                    borderRadius: 10,
                    marginBottom: i < dailyServices.length - 1 ? 12 : 0,
                  }}
                >
                  <div>
...
const Invoices = ({ T, dailyServices }) => {
  ...
  const create = async () => {
    ...
    if (invoiceMode === 'free' && !form.freeClientName.trim())
      return notify('Le nom du client est requis.', 'error')
  ...
  const openPDF = (inv) => {
    ...
    setVisibleFields({
      invoice_date: true,
      av_name: !!(avData?.full_name),
      av_email: !!(avData?.email),
      av_siret: !!(avData?.siret),
      av_tva: !!(avData?.tva_number),
      av_bank: !!(avData?.bank_details),
      client_name: true,
    ...
    return (
      <div className="main-content" style={{ padding: '32px 40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          <Btn variant="ghost" onClick={() => setPdfInvoice(null)} T={T}>
            ← Retour
          </Btn>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: T.text }}>
            Aperçu — {pdfInvoice.invoice_number}
          </h1>
        </div>
        ...
        {(!avData?.full_name || !avData?.siret) && (
          ...
          >
            <span>⚠️</span>
            <span>
              Certaines informations de ton profil sont manquantes. Va dans{' '}
              <strong>Paramètres</strong> pour les compléter.
            </span>
          </div>
        )}
        ...
        >
          <Card T={T}>
            <div style={{ fontWeight: 700, fontSize: 16, color: T.text, marginBottom: 14 }}>
              Informations incluses
            </div>
            <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 14 }}>
              Clique pour masquer/afficher dans le PDF.
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {fields.map((f) => {
                ...
                return (
                  <div
                    key={f.key}
                    onClick={() => setVisibleFields((v) => ({ ...v, [f.key]: !visible }))}
                    style={{
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'space-between',
                      padding: '10px 12px',
                      borderRadius: 10,
                      background: visible ? T.accentLight : T.surfaceHigh,
                      border: `1px solid ${visible ? T.accent + '40' : T.border}`,
                      cursor: 'pointer',
                      transition: 'all 0.2s',
                    }}
                    onMouseEnter={(e) => {
                  ...
                  >
                    ...
                    {f.warn && visible && (
                      <span style={{ fontSize: 10, color: T.warning, marginRight: 6 }}>!</span>
                    )}
                    <span
                      style={{
                        fontSize: 10,
                        color: visible ? T.accent : T.textDim,
                        fontWeight: 600,
                        flexShrink: 0,
                        marginLeft: 8,
                      }}
            ...
            <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Btn full onClick={() => generatePDF(pdfInvoice, pdfClient, avData, visibleFields)} T={T}>
                ⬇ Télécharger / Imprimer
              </Btn>
              <Btn full variant="success" onClick={() => sendByEmail(pdfInvoice, pdfClient)} T={T}>
                ✉ Envoyer par email
              </Btn>
              <Btn full variant="ghost" onClick={() => setPdfInvoice(null)} T={T}>
                Fermer
              </Btn>
            </div>
          </Card>
            ...
            >
              ...
              >
                <div style={{ fontSize: 24, fontWeight: 800 }}>
                  <span style={{ color: '#059669' }}>VA</span> Billing
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 28, fontWeight: 800 }}>FACTURE</div>
                  <div style={{ fontSize: 13, color: '#6B6963', fontFamily: 'monospace' }}>
              ...
              >
                <div>
                  ...
                  {visibleFields.av_name !== false && (
                    <div style={{ fontWeight: 700, marginBottom: 4, fontSize: 14 }}>
                      {avData?.full_name || (
                        <span style={{ color: '#DC2626', fontStyle: 'italic' }}>Nom non renseigné</span>
                      )}
                    </div>
                  )}
                  <div style={{ fontSize: 12, color: '#6B6963', lineHeight: 1.8 }}>
                    ...
                    {visibleFields.av_tva !== false && avData?.tva_number && (
                      <div>TVA : {avData.tva_number}</div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {visibleFields.client_name !== false && (
                    <div style={{ fontWeight: 700, marginBottom: 4, fontSize: 14 }}>{clientName}</div>
                  )}
                  <div style={{ fontSize: 12, color: '#6B6963', lineHeight: 1.8 }}>
                    {visibleFields.client_company !== false && pdfClient?.company && (
                      <div>{pdfClient.company}</div>
                    )}
                    {visibleFields.client_email !== false && clientEmail && <div>{clientEmail}</div>}
                    {visibleFields.client_phone !== false && pdfClient?.phone && (
                      <div>Tél : {pdfClient.phone}</div>
                    )}
                    {visibleFields.client_vat !== false && pdfClient?.vat_id && (
                      <div>TVA : {pdfClient.vat_id}</div>
                    )}
                  </div>
                </div>
              </div>
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600 }}>
                      {pdfInvoice.date || new Date().toLocaleDateString('fr')}
                    </div>
                  </div>
                  <div>
                    <div
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600 }}>{pdfInvoice.due_date || '—'}</div>
                  </div>
                </div>
              )}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 20 }}>
                <thead>
                  <tr style={{ background: '#1A1916', color: '#fff' }}>
                    ...
                    <th style={{ padding: '11px 14px', textAlign: 'right', fontSize: 10 }}>QTÉ</th>
                    <th style={{ padding: '11px 14px', textAlign: 'right', fontSize: 10 }}>TARIF</th>
                    <th style={{ padding: '11px 14px', textAlign: 'right', fontSize: 10 }}>MONTANT</th>
                  </tr>
                </thead>
                <tbody>
                  {(pdfInvoice.items || []).map((item, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #E0DED8' }}>
                      ...
                      >
                        {Number(item.qty || item.duration || 0).toFixed(2)} {item.unit || 'h'}
                      </td>
                      <td
                        style={{
                          padding: '11px 14px',
                      ...
                      >
                        {(Number(item.qty || item.duration || 0) * Number(item.rate || 0)).toFixed(2)} €
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <div style={{ width: 220, borderTop: '2px solid #1A1916', paddingTop: 12 }}>
                  ...
                  >
                    ...
                    <span style={{ fontFamily: 'monospace' }}>
                      {Number(htAmount || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €
                    </span>
                  </div>
                  {tvaRate > 0 ? (
                    <div
                    ...
                    >
                      ...
                      <span style={{ fontFamily: 'monospace' }}>
                        {Number(tvaAmount || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €
                      </span>
                    </div>
                  ) : (
                    <div
                  ...
                  >
                    ...
                    <span style={{ color: '#059669', fontFamily: 'monospace' }}>
                      {Number(ttcAmount || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €
                    </span>
                  </div>
                </div>
              </div>
    ...
    <div id="invoices" className="main-content" style={{ padding: '32px 40px' }}>
      ...
      {showForm && (
        <Card accent T={T} style={{ marginBottom: 24 }} className="fade-up">
          <div style={{ fontWeight: 700, marginBottom: 20, fontSize: 18, color: T.text }}>
            Nouvelle facture
          </div>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  padding: '14px 18px',
                  borderRadius: 12,
                  border: `1px solid ${invoiceMode === m.id ? T.accent : T.border}`,
                  background: invoiceMode === m.id ? T.accentLight : 'transparent',
                  color: invoiceMode === m.id ? T.accent : T.textMuted,
                  fontSize: 14,
                  fontWeight: invoiceMode === m.id ? 600 : 500,
                  cursor: 'pointer',
                  fontFamily: 'inherit',
                  transition: 'all 0.2s',
                  textAlign: 'left',
          ...
          >
            {invoiceMode === 'hours' ? (
              ...
              <Input
                label="Nom du client *"
                value={form.freeClientName}
                onChange={(v) => setForm({ ...form, freeClientName: v })}
            ...
            {invoiceMode === 'free' && (
              <Input
                label="Email client (pour envoi)"
                value={form.freeClientEmail}
                onChange={(v) => setForm({ ...form, freeClientEmail: v })}
          ...
          >
            ...
            >
              <Input
                ...
                type="number"
                value={form.tvaRate}
                onChange={(v) => setForm({ ...form, tvaRate: v })}
                placeholder="21"
                suffix="%"
                T={T}
              />
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                <div style={{ fontSize: 13, color: T.textMuted, fontWeight: 600 }}>
                  Heures non facturées — {client?.name}
                </div>
                {sessions.length > 0 && (
                  <button
                    onClick={toggleAll}
              ...
              {loadingSessions ? (
                ...
                >
                  Aucune heure non facturée pour ce client.
                </div>
              ) : (
                <div
                  style={{
                  ...
                  {sessions.map((s, i) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseEnter={(e) => {
                          if (!checked) e.currentTarget.style.background = T.surfaceHigh
                        }}
                        onMouseLeave={(e) => {
              ...
              {selectedSessions.length > 0 && (
                ...
                >
                  <span style={{ fontSize: 13, color: T.accent, fontWeight: 600 }}>
                    {selectedSessions.length} session(s) · {hoursTotal.toFixed(2)}h · {client?.rate}€/h
                  </span>
                  <span className="mono" style={{ fontSize: 16, color: T.accent, fontWeight: 700 }}>
                    {hoursAmount.toFixed(2)} €
                  </span>
                </div>
              )}
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                <div
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '2fr 90px 110px 100px 40px',
                    gap: 10,
                    padding: '10px 14px',
                    borderBottom: `1px solid ${T.border}`,
                  }}
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        gap: 10,
                        padding: '10px 14px',
                        borderBottom: i < freeItems.length - 1 ? `1px solid ${T.border}` : 'none',
                        alignItems: 'center',
                      }}
                    >
                      <input
                        value={item.service}
                        onChange={(e) => updateFreeItem(i, 'service', e.target.value)}
                        placeholder="Ex: Mise en page 30 docs"
                        style={{
                          background: T.surface,
                          border: `1px solid ${T.border}`,
                          borderRadius: 8,
                      ...
                      <input
                        type="number"
                        value={item.qty || ''}
                        onChange={(e) => updateFreeItem(i, 'qty', e.target.value)}
                        placeholder="1"
                        style={{
                          background: T.surface,
                          border: `1px solid ${T.border}`,
                      ...
                      <div style={{ position: 'relative' }}>
                        <input
                          type="number"
                          value={item.rate || ''}
                          onChange={(e) => updateFreeItem(i, 'rate', e.target.value)}
                          placeholder="25"
                          style={{
                            width: '100%',
                            background: T.surface,
                      ...
                      <select
                        ...
                        onChange={(e) => updateFreeItem(i, 'unit', e.target.value)}
                        style={{
                          background: T.surface,
                          border: `1px solid ${T.border}`,
                          borderRadius: 8,
                      ...
                      <button
                        onClick={() => setFreeItems((e) => e.filter((_, j) => j !== i))}
                        disabled={freeItems.length === 1}
                        style={{
                          background: 'none',
                          border: 'none',
                          color: freeItems.length === 1 ? T.textDim : T.danger,
                          cursor: freeItems.length === 1 ? 'not-allowed' : 'pointer',
                          fontSize: 16,
                          padding: 4,
                        }}
                      >
              ...
              <Btn small variant="subtle" onClick={addFreeItem} T={T}>
                + Ajouter une ligne
              </Btn>
            </div>
          )}
          ...
          >
            <div>
              <div className="mono" style={{ fontSize: 24, fontWeight: 600, color: T.text }}>
                Total TTC : <span style={{ color: T.accent }}>{grandTotal.toFixed(2)} €</span>
              </div>
              <div style={{ fontSize: 12, color: T.textMuted, marginTop: 4 }}>
                HT: {htTotal.toFixed(2)} €{' '}
                {tvaRate > 0 ? `· TVA (${tvaRate}%): ${tvaAmount.toFixed(2)} €` : '· TVA: N/A'}
              </div>
      ...
      {loading ? (
        ...
        <>
          <Card
            ...
            className="fade-up s2 invoice-table"
          >
                  ...
                  >
                    ...
                    <td
                      style={{
                        ...
                        fontSize: 13,
                        color:
                          inv.due_date && new Date(inv.due_date) < new Date() ? T.danger : T.textMuted,
                      }}
                    >
                      {inv.due_date || '—'}
                    </td>
                    ...
                    <td style={{ padding: '14px 18px' }}>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <Btn small variant="subtle" onClick={() => openPDF(inv)} T={T}>
                          PDF
                        </Btn>
                        <Btn
                          small
                        ...
                        {inv.status !== 'payée' && (
                          <Btn
                            small
                            variant="subtle"
                            onClick={() => changeStatus(inv.id, 'payée')}
                            T={T}
                          >
                            ✓
                          </Btn>
                        )}
                        <Btn small variant="danger" onClick={() => del(inv.id)} T={T}>
                          ✕
                        </Btn>
                      </div>
                    </td>
          ...
          >
            {filtered.map((inv) => (
              <Card key={inv.id} T={T}>
                ...
                >
                  <div>
                    <div className="mono" style={{ fontSize: 14, fontWeight: 600, color: T.accent }}>
                      {inv.invoice_number}
                    </div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: T.text, marginTop: 4 }}>
                      {inv.clients?.name}
                    </div>
                  </div>
                  <Badge status={inv.status} T={T} />
                ...
                <div
                  className="mono"
                  style={{ fontSize: 26, fontWeight: 600, color: T.text, marginBottom: 14 }}
                >
                  {Number(inv.amount).toLocaleString('fr')} €
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  <Btn
                    small
                    variant="success"
                    onClick={() => sendByEmail(inv, clients.find((x) => x.id === inv.client_id))}
                    T={T}
                  >
                    ✉ Email
                  </Btn>
...
const Quotes = ({ T }) => {
  ...
  const save = async () => {
    if (!form.prospectName.trim())
      return notify('Le nom du destinataire est requis.', 'error')
    if (total === 0)
      return notify("Ajoute au moins une prestation avec un tarif.", 'error')
    ...
    <div className="main-content" style={{ padding: '32px 40px' }}>
      ...
      >
        <div>
          ...
          <p style={{ color: T.textMuted, fontSize: 15 }}>
            Crée un devis libre — pas besoin d'un client existant.
          </p>
        </div>
        <Btn onClick={() => setShowForm(!showForm)} T={T} icon="📋">
          Nouveau devis
      ...
      >
        <span>ℹ️</span>
        <span>
          Un devis est <strong>libre</strong> — tu n'as pas besoin que le prospect soit dans ta liste de
          clients.
        </span>
      </div>
        ...
        <Card accent T={T} style={{ marginBottom: 24 }} className="fade-up">
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              <Input
                ...
                onChange={(v) => setForm({ ...form, prospectName: v })}
                placeholder="Jean Martin"
                T={T}
              />
              <Input
                ...
                onChange={(v) => setForm({ ...form, prospectEmail: v })}
                placeholder="jean@email.com"
                T={T}
              />
              <Input
                ...
                onChange={(v) => setForm({ ...form, prospectCompany: v })}
              ...
              <Input
                ...
                onChange={(v) => setForm({ ...form, validDays: Number(v) })}
                placeholder="30"
                T={T}
              />
            </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '2fr 100px 120px 120px 40px',
                  gap: 10,
                  padding: '10px 16px',
                  borderBottom: `1px solid ${T.border}`,
                }}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 10,
                      padding: '10px 16px',
                      borderBottom: i < form.items.length - 1 ? `1px solid ${T.border}` : 'none',
                      alignItems: 'center',
                    }}
                  >
                    <div style={{ display: 'flex', gap: 10 }}>
                      <select
                        ...
                        style={{
                          ...
                          borderRadius: 8,
                          padding: '9px 12px',
                          color: item.service ? T.text : T.textDim,
                          fontSize: 13,
                          flex: 1,
                          fontFamily: 'inherit',
                        }}
                      ...
                      {item.service === 'custom' && (
                        <input
                          ...
                          onChange={(e) => updateItem(i, 'customService', e.target.value)}
                          placeholder="Écrire le service..."
                          style={{
                            background: T.surface,
                            border: `1px solid ${T.border}`,
                            borderRadius: 8,
                    ...
                    <div
                      ...
                      style={{
                        fontSize: 14,
                        fontWeight: 600,
                        color: lineTotal > 0 ? T.accent : T.textDim,
                        paddingLeft: 6,
                      }}
                    >
                    ...
                    <button
                      ...
                      style={{
                        background: 'none',
                        border: 'none',
                        color: form.items.length === 1 ? T.textDim : T.danger,
            ...
            <Btn small variant="subtle" onClick={addItem} T={T}>
              + Ajouter une ligne
            </Btn>
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <textarea
              ...
              placeholder="Conditions particulières, délai de réalisation, etc."
              rows={3}
              style={{
                width: '100%',
                background: T.surfaceHigh,
          ...
          >
            <div>
              <div className="mono" style={{ fontSize: 26, fontWeight: 600, color: T.text }}>
                Total TTC : <span style={{ color: T.accent }}>{total.toFixed(2)} €</span>
              </div>
              <div style={{ fontSize: 12, color: T.textMuted, marginTop: 4 }}>
                HT: {htTotal.toFixed(2)} €{' '}
                {tvaRate > 0 ? `· TVA (${tvaRate}%): ${tvaAmount.toFixed(2)} €` : '· TVA: N/A'}
              ...
              <div style={{ fontSize: 12, color: T.textMuted, marginTop: 4 }}>
                {form.items.filter((i) => i.service && Number(i.rate) > 0).length} prestation(s) ·
                valable {form.validDays} jours
              </div>
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <Btn
                onClick={save}
                loading={saving}
                disabled={!form.prospectName || total === 0}
                T={T}
              >
                Créer le devis
              </Btn>
              <Btn variant="ghost" onClick={() => setShowForm(false)} T={T}>
                Annuler
              </Btn>
            </div>
          </div>
                ...
                >
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                    <div>
                      ...
                      >
                        <span className="mono" style={{ fontSize: 14, fontWeight: 700, color: T.accent }}>
                          {q.ref || 'DEV-?'}
                        </span>
                        <span
                          style={{
                            ...
                            padding: '3px 12px',
                            borderRadius: 20,
                            background: T[sc.bg] || T.surfaceHigh,
                            color: T[sc.color] || T.textMuted,
                          }}
                        >
                          {q.status}
                        </span>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 600, color: T.text }}>
                        {q.prospect_name || q.clients?.name || '—'}
                      </div>
                      {q.prospect_company && (
                        <div style={{ fontSize: 13, color: T.textMuted }}>{q.prospect_company}</div>
                      )}
                      <div style={{ fontSize: 12, color: T.textDim, marginTop: 6 }}>
                        {new Date(q.created_at).toLocaleDateString('fr')} ·{' '}
                        {Array.isArray(q.items) ? q.items.length : 0} prestation(s)
                      </div>
                    </div>
                  </div>
                  <div
                  ...
                  >
                    <span
                      className="mono"
                      style={{ fontSize: 22, fontWeight: 700, color: T.accent }}
                    >
                      {Number(q.amount).toLocaleString('fr')} €
                    </span>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {q.prospect_email && (
                        <Btn small variant="subtle" onClick={() => sendQuote(q)} T={T}>
                          ✉ Email
                        </Btn>
                      )}
                      {q.status === 'brouillon' && (
                        <Btn
                          small
                          variant="subtle"
                          onClick={() => changeStatus(q.id, 'envoyé')}
                          T={T}
                        >
                          Marquer envoyé
                        </Btn>
                      ...
                      {q.status === 'envoyé' && (
                        <Btn
                          small
                          variant="success"
                          onClick={() => changeStatus(q.id, 'accepté')}
                          T={T}
                        >
                          ✓ Accepté
                        </Btn>
                      ...
                      {q.status === 'envoyé' && (
                        <Btn
                          small
                          variant="danger"
                          onClick={() => changeStatus(q.id, 'refusé')}
                          T={T}
                        >
                          ✕ Refusé
                        </Btn>
                      )}
                      <Btn small variant="danger" onClick={() => del(q.id)} T={T}>
                        ✕
                      </Btn>
                    </div>
                  </div>
...
const generateQuotePDF = (quote, settings) => {
  ...
  const html = `
    ...
    <body>
      ...
      <div class="validity">
        ✓ Ce devis est valable ${quote.valid_days || 30} jours à compter de la date d'émission.
      </div>
...
export default function App() {
  ...
  const timerActions = {
    ...
    stop: async (saveFn) => {
      ...
      if (saveFn && sec >= 5) {
        try {
          const data = await saveFn(sec, cId, tsk)
          return data
        } catch (e) {
          console.error(e)
          return null
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: '50%',
                  flexShrink: 0,
                  background: timerPaused ? T.warning : '#F87171',
                  animation: timerPaused ? 'none' : 'blink 0.8s ease infinite',
                }}
              />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span
                  style={{
                    fontSize: 11,
                    color: timerPaused ? T.warning : '#A8A29E',
                    fontWeight: 500,
                    lineHeight: 1,
                  }}
                >
                ...
                <span
                  style={{
                    fontSize: 14,
                    color: timerPaused ? T.warning : '#FAFAF9',
                    fontWeight: 600,
                    lineHeight: 1,
                  }}
                >
