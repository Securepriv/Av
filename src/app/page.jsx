      ...
      <div className="fade-up s2 grid-2" style={{display:'grid',gridTemplateColumns:'1.6fr 1fr',gap:16}}>
        <Card T={T}>
          ...
          <div style={{fontSize:12,color:T.info,background:T.infoBg,borderRadius:8,padding:'8px 12px',marginBottom:14,lineHeight:1.6}}>
            ℹ️ Factures avec statut <strong>Envoyée</strong> ou <strong>En retard</strong> — transmises au client mais pas encore réglées. Les brouillons et les payées ne s'affichent pas ici.
          </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          {(stats?.pendingCount||0)>0&&(
            <Card T={T} style={{background:T.warningBg,borderColor:`${T.warning}30`}}>
              ...
              <div style={{fontSize:12,color:T.textMuted,lineHeight:1.6}}>{stats.pendingCount} facture(s) non réglée(s) · <strong style={{color:T.text}}>{(stats.pending||0).toLocaleString('fr')} €</strong></div>
            </Card>
          )}
        </div>
      </div>
            ...
            <Card T={T}>
              ...
              :paidSessions.map((s,i)=>(
                <div key={s.id} style={{display:'flex',justifyContent:'space-between',padding:'9px 0',borderBottom:i<paidSessions.length-1?`1px solid ${T.border}`:'none'}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:500,color:T.text}}>{s.task||'Tâche'}</div>
                    <div style={{fontSize:11,color:T.textMuted,marginTop:1}}>{s.date}</div>
                  </div>
                  <div style={{textAlign:'right'}}>
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  <div class="header">
    <div class="logo"><span>VA</span>Billing</div>
    <div style="text-align:right"><h1>DEVIS</h1><div class="ref">${quote.ref||'DEV-001'}</div></div>
  </div>
  <div class="parties">
    <div>
  ...
  <div class="info-box">
    ...
    <div class="info-item"><label>Validité</label><span>30 jours</span></div>
  </div>
  <div class="validity">✓ Ce devis est valable 30 jours à compter de la date d'émission.</div>
  <table>
    <thead><tr>
      <th>Prestation</th><th style="text-align:right">Quantité</th><th style="text-align:right">Tarif unitaire</th><th style="text-align:right">Montant</th>
    </tr></thead>
    <tbody>
  ...
  <div class="totals"><div class="totals-box">
    <div class="total-row"><span>Sous-total HT</span><span>${fmt(quote.amount)} €</span></div>
    <div class="total-row total-grand"><span>TOTAL</span><span>${fmt(quote.amount)} €</span></div>
  </div></div>
  ${quote.note?`<div class="note"><strong>Note :</strong> ${quote.note}</div>`:''}
  ...
  return(
    <div className="main-content" style={{padding:'28px 32px'}}>
      ...
      <div className="fade-up s1" style={{padding:'10px 16px',background:T.infoBg,borderRadius:10,fontSize:13,color:T.info,marginBottom:20,lineHeight:1.6}}>
        ℹ️ Un devis est <strong>libre</strong> — tu n'as pas besoin que le prospect soit dans ta liste de clients. Tu remplis son nom, email et les prestations demandées. Une fois accepté, tu crées la facture depuis la section Factures.
      </div>
        ...
        <Card accent T={T} style={{marginBottom:20}} className="fade-up">
                ...
                return(
                  <div key={i} style={{display:'grid',gridTemplateColumns:'2fr 100px 120px 120px 36px',gap:8,padding:'8px 14px',borderBottom:i<form.items.length-1?`1px solid ${T.border}`:'none',alignItems:'center'}}>
                    ...
                    <input type="number" placeholder="1" value={item.qty||''} onChange={e=>updateItem(i,'qty',e.target.value)}
                      style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:7,padding:'7px 10px',color:T.text,fontSize:13,textAlign:'center'}}/>
          ...
          <div style={{marginBottom:16}}>
            ...
            <textarea value={form.note} onChange={e=>setForm({...form,note:e.target.value})} placeholder="Conditions particulières, délai de réalisation, etc." rows={3}
              style={{width:'100%',background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'10px 13px',color:T.text,fontSize:13,resize:'vertical',fontFamily:'inherit'}}/>
          </div>

          {/* Total + actions */}
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',borderTop:`1px solid ${T.border}`,paddingTop:16}}>
            <div>
              <div className="mono" style={{fontSize:22,fontWeight:600,color:T.text}}>Total : <span style={{color:T.accent}}>{total.toFixed(2)} €</span></div>
