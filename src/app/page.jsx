    ...
    try {
      ...
      if (status === 'payée') {
        ...
        if (invoice?.items && Array.isArray(invoice.items)) {
          ...
          if (sessionIds.length > 0) {
            const { data: updatedSes, error: sesErr, count, status: sesStatus } = await supabase
              .from('sessions').update({ invoiced: true }, { count: 'exact' }).in('id', sessionIds).select()
            console.log('Update sessions résultat:', { updatedSes, sesErr, count, sesStatus })
          }
