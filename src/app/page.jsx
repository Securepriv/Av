...
const Dashboard = ({ setPage, user, T }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const [invRes, sesRes, cliRes, sesHeatRes, futureDueRes] = await Promise.all([
          supabase.from('invoices').select('*,clients(name)').gte('created_at', twelveMonthsAgo).order('created_at', { ascending: false }),
          supabase.from('sessions').select('duration,date').gte('date', weekAgo),
          supabase.from('clients').select('id', { count: 'exact', head: true }),
          supabase.from('sessions').select('date,duration').gte('date', oneYearAgo),
          supabase.from('invoices').select('amount,due_date,status').gte('due_date', now.toISOString().split('T')[0]).in('status', ['envoyée', 'en retard']),
        ])
        ...
        // Stats mois en cours
        const thisMonthInv = allInv.filter((i) => i.created_at >= firstDayThisMonth)
        const lastMonthInv = allInv.filter((i) => i.created_at >= firstDayLastMonth && i.created_at <= lastDayLastMonth)
        const pending = allInv.filter((i) => i.status === 'envoyée' || i.status === 'en retard')

        const thisMonthBilled = thisMonthInv.filter((i) => i.status === 'payée').reduce((s, i) => s + Number(i.amount), 0)
        const lastMonthBilled = lastMonthInv.filter((i) => i.status === 'payée').reduce((s, i) => s + Number(i.amount), 0)
        const momVariance = lastMonthBilled > 0 ? ((thisMonthBilled - lastMonthBilled) / lastMonthBilled * 100) : null

        const thisMonthCount = thisMonthInv.filter((i) => i.status === 'payée').length
        const lastMonthCount = lastMonthInv.filter((i) => i.status === 'payée').length
        const momCountVariance = lastMonthCount > 0 ? ((thisMonthCount - lastMonthCount) / lastMonthCount * 100) : null

        setStats({
          billed: thisMonthBilled, lastMonthBilled,
          momVariance, momCountVariance,
          pending: pending.reduce((s, i) => s + Number(i.amount), 0),
          pendingCount: pending.length,
          weekHours: weekH.toFixed(1),
          clients: cliRes.count || 0,
          totalPaid: allInv.filter((i) => i.status === 'payée').reduce((s, i) => s + Number(i.amount), 0),
        })
        ...
        for (let i = 11; i >= 0; i--) {
          ...
          const revenue = allInv
            .filter((inv) => inv.created_at >= created && inv.created_at < nextMonth && inv.status === 'payée')
            .reduce((s, inv) => s + Number(inv.amount), 0)
          months.push({ key, label, revenue, isCurrentMonth: i === 0 })
        }
        setMonthlyRevenue(months)

        // Heatmap sessions (365 jours)
        const hmap = {}
        ;(sesHeatRes.data || []).forEach((s) => {
          hmap[s.date] = (hmap[s.date] || 0) + Number(s.duration)
        })
        setHeatmapData(hmap)

        // Cashflow prévisionnel (30 prochains jours)
        const cfDays = []
        for (let i = 0; i <= 30; i++) {
          const d = new Date(now)
          d.setDate(d.getDate() + i)
          const dStr = d.toISOString().split('T')[0]
          const due = (futureDueRes.data || [])
            .filter((inv) => inv.due_date === dStr)
            .reduce((s, inv) => s + Number(inv.amount), 0)
          cfDays.push({ date: dStr, label: i === 0 ? "Auj." : i % 5 === 0 ? `J+${i}` : '', amount: due })
        }
        setCashflow(cfDays)
  ...
  const maxHeat = Math.max(...Object.values(heatmapData), 0.1)

  const heatColor = (val) => {
    if (!val) return T.surfaceHigh
    const intensity = Math.min(val / maxHeat, 1)
    if (intensity < 0.25) return T.accent + '40'
    if (intensity < 0.5) return T.accent + '80'
    if (intensity < 0.75) return T.accent + 'BB'
    return T.accent
  }

  // ── Cashflow cumul ──
  let cumul = 0
  const cfCumul = cashflow.map((d) => { cumul += d.amount; return { ...d, cumul } })
  const maxCumul = Math.max(cumul, 1)
  const hasCashflow = cashflow.some((d) => d.amount > 0)

  // ── Variance badge ──
  const VarBadge = ({ val, suffix = '%' }) => {
    if (val === null || val === undefined) return <span style={{ fontSize: 11, color: T.textDim }}>— vs mois dernier</span>
    const pos = val >= 0
    return (
      <span style={{ fontSize: 11, fontWeight: 700, color: pos ? T.success : T.danger, background: pos ? T.successBg : T.dangerBg, padding: '2px 8px', borderRadius: 20 }}>
        {pos ? '▲' : '▼'} {Math.abs(val).toFixed(1)}{suffix} vs mois dernier
      </span>
    )
  }
  ...
  return (
    <div className="main-content" style={{ padding: '32px 40px' }}>
      ...
      <div className="fade-up" style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: T.textMuted, fontSize: 15 }}>Vue d'ensemble de ton activité — mis à jour en temps réel.</p>
      </div>

      {/* ── KPI Cards ── */}
      <div className="fade-up s1 stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 28 }}>
        {[
          {
            label: 'Encaissé ce mois', icon: '💰', color: T.accent,
            value: `${(stats?.billed || 0).toLocaleString('fr')} €`,
            sub: <VarBadge val={stats?.momVariance} />,
          },
          {
            label: 'En attente', icon: '⏳', color: T.warning,
            value: `${(stats?.pending || 0).toLocaleString('fr')} €`,
            sub: <span style={{ fontSize: 11, color: T.textDim }}>{stats?.pendingCount || 0} facture(s) non réglée(s)</span>,
          },
          {
            label: 'Heures / semaine', icon: '⏱️', color: T.info,
            value: `${stats?.weekHours || 0}h`,
            sub: <span style={{ fontSize: 11, color: T.textDim }}>7 derniers jours</span>,
          },
          {
            label: 'Total encaissé', icon: '📈', color: T.success,
            value: `${(stats?.totalPaid || 0).toLocaleString('fr')} €`,
            sub: <span style={{ fontSize: 11, color: T.textDim }}>{stats?.clients || 0} client(s) actifs</span>,
          },
        ].map((s) => (
          <Card key={s.label} T={T} style={{ position: 'relative', overflow: 'hidden' }}>
            <div style={{ fontSize: 22, marginBottom: 10, opacity: 0.85 }}>{s.icon}</div>
            <div style={{ fontSize: 10, color: T.textMuted, fontWeight: 700, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{s.label}</div>
            <div className="mono" style={{ fontSize: 26, fontWeight: 700, color: s.color, lineHeight: 1, marginBottom: 8 }}>{s.value}</div>
            <div>{s.sub}</div>
            <div style={{ position: 'absolute', right: -16, top: -16, width: 80, height: 80, borderRadius: '50%', background: s.color, opacity: 0.07 }} />
          </Card>
        ))}
      </div>

      {/* ── Revenue Chart + Heatmap ── */}
      <div className="fade-up s2" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 20, marginBottom: 20 }}>

        {/* Revenue 12 mois */}
        <Card T={T}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <div>
              <div style={{ fontWeight: 700, fontSize: 16, color: T.text }}>Revenus — 12 derniers mois</div>
              <div style={{ fontSize: 12, color: T.textMuted, marginTop: 3 }}>Factures payées par mois</div>
            </div>
            <VarBadge val={stats?.momVariance} />
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 140, paddingBottom: 24, position: 'relative' }}>
            {/* Lignes de grille */}
            {[0.25, 0.5, 0.75, 1].map((pct) => (
              <div key={pct} style={{ position: 'absolute', left: 0, right: 0, bottom: 24 + pct * 116, borderTop: `1px dashed ${T.border}`, display: 'flex', alignItems: 'center' }}>
                <span style={{ fontSize: 9, color: T.textDim, paddingRight: 4, background: T.surface, marginLeft: 'auto', lineHeight: 1 }}>
                  {(maxRev * pct / 1000).toFixed(0)}k
                </span>
              </div>
            ))}
            {monthlyRevenue.map((m) => {
              const h = maxRev > 0 ? (m.revenue / maxRev) * 116 : 2
              return (
                <div key={m.key} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, zIndex: 1 }}>
                  <div
                    title={`${m.label} : ${m.revenue.toLocaleString('fr')} €`}
                    style={{
                      width: '100%', height: Math.max(h, 2), borderRadius: '4px 4px 0 0',
                      background: m.isCurrentMonth ? T.gradient : m.revenue > 0 ? T.accent + '60' : T.surfaceHigh,
                      transition: 'height 0.4s ease',
                      cursor: 'default',
                      boxShadow: m.isCurrentMonth ? `0 0 8px ${T.accent}40` : 'none',
                    }}
                  />
                  <span style={{ fontSize: 8, color: m.isCurrentMonth ? T.accent : T.textDim, fontWeight: m.isCurrentMonth ? 700 : 400, whiteSpace: 'nowrap' }}>{m.label}</span>
                </div>
              )
            })}
          </div>
          {/* Comparaison mois */}
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 14, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {[
              { label: 'Ce mois', val: stats?.billed, color: T.accent },
              { label: 'Mois dernier', val: stats?.lastMonthBilled, color: T.textMuted },
              { label: 'Variation', val: stats?.momVariance !== null ? `${stats?.momVariance >= 0 ? '+' : ''}${stats?.momVariance?.toFixed(1)}%` : '—', color: stats?.momVariance >= 0 ? T.success : T.danger, raw: true },
            ].map((item) => (
              <div key={item.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 10, color: T.textMuted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>{item.label}</div>
                <div className="mono" style={{ fontSize: 15, fontWeight: 700, color: item.color }}>
                  {item.raw ? item.val : `${(item.val || 0).toLocaleString('fr')} €`}
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Heatmap activité */}
        <Card T={T}>
          <div style={{ fontWeight: 700, fontSize: 16, color: T.text, marginBottom: 4 }}>Activité — 365 jours</div>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 16 }}>Heures travaillées par jour</div>
          <div style={{ overflowX: 'auto' }}>
            <div style={{ display: 'flex', gap: 2, minWidth: 'fit-content' }}>
              {/* Grouper par semaine */}
              {Array.from({ length: 53 }, (_, weekIdx) => {
                const weekDays = heatDays.slice(weekIdx * 7, weekIdx * 7 + 7)
                return (
                  <div key={weekIdx} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {weekDays.map((day) => {
                      ...
                      return (
                        <div
                          key={day}
                          title={val ? `${day} : ${val.toFixed(1)}h` : day}
                          style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 12 }}>
            ...
            {['#00000000', '40', '80', 'BB', 'FF'].map((op, i) => (
              <div key={i} style={{ width: 10, height: 10, borderRadius: 2, background: i === 0 ? T.surfaceHigh : T.accent + op }} />
            ))}
            <span style={{ fontSize: 10, color: T.textDim }}>Plus</span>
            <span style={{ marginLeft: 'auto', fontSize: 11, color: T.textMuted, fontWeight: 600 }}>
              {Object.keys(heatmapData).length} jours actifs
            </span>
          </div>
          {/* Stats rapides heatmap */}
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${T.border}`, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              { label: 'Total heures', val: `${Object.values(heatmapData).reduce((a, b) => a + b, 0).toFixed(1)}h` },
              { label: 'Meilleur jour', val: `${Math.max(...Object.values(heatmapData), 0).toFixed(1)}h` },
            ].map((item) => (
              <div key={item.label} style={{ background: T.surfaceHigh, borderRadius: 8, padding: '8px 12px' }}>
                <div style={{ fontSize: 10, color: T.textMuted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{item.label}</div>
                <div className="mono" style={{ fontSize: 16, fontWeight: 700, color: T.accent, marginTop: 2 }}>{item.val}</div>
              </div>
            ))}
          </div>
        </Card>
      ...
      {/* ── Cashflow + Pending + Actions ── */}
      <div className="fade-up s3" style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr 0.8fr', gap: 20 }}>

        {/* Cashflow prévisionnel */}
        <Card T={T}>
          <div style={{ fontWeight: 700, fontSize: 16, color: T.text, marginBottom: 4 }}>Cashflow prévu</div>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 16 }}>Entrées attendues — 30 prochains jours</div>
          {!hasCashflow ? (
            <Empty icon="📅" text="Aucune échéance" sub="Aucune facture due dans les 30 prochains jours." T={T} />
          ) : (
            <>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 80, marginBottom: 8 }}>
                {cfCumul.map((d, i) => {
                  const h = (d.cumul / maxCumul) * 76
                  return (
                    <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                      <div
                        title={d.amount > 0 ? `${d.date} : +${d.amount.toLocaleString('fr')} €` : ''}
                        style={{
                          width: '100%', height: Math.max(h, 1), borderRadius: '2px 2px 0 0',
                          background: d.amount > 0 ? T.info : T.surfaceHigh,
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: T.textDim, marginBottom: 16 }}>
                <span>Auj.</span><span>J+15</span><span>J+30</span>
              </div>
              <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: T.textMuted, marginBottom: 6 }}>
                  <span>Total attendu</span>
                  <span className="mono" style={{ fontWeight: 700, color: T.info }}>{maxCumul.toLocaleString('fr')} €</span>
                </div>
                {cfCumul.filter((d) => d.amount > 0).slice(0, 3).map((d) => (
                  <div key={d.date} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: T.textMuted, padding: '4px 0' }}>
                    <span>{new Date(d.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span>
                    <span className="mono" style={{ color: T.info, fontWeight: 600 }}>+{d.amount.toLocaleString('fr')} €</span>
                  </div>
                ))}
              </div>
            </>
        ...
        <Card T={T}>
          ...
          {pendingInvoices.length === 0 ? (
            <Empty icon="✅" text="Tout est réglé !" sub="Aucune facture en attente." T={T} />
          ) : (
            pendingInvoices.map((inv, i) => {
              const isLate = inv.status === 'en retard' || (inv.due_date && new Date(inv.due_date) < new Date())
              return (
                <div key={inv.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i < pendingInvoices.length - 1 ? `1px solid ${T.border}` : 'none' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {inv.clients?.name} · éch. <span style={{ color: isLate ? T.danger : T.textMuted }}>{inv.due_date || '—'}</span>
                    </div>
                  </div>
          ...
          {stats?.pendingCount > 0 && (
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${T.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 12, color: T.textMuted }}>{stats.pendingCount} facture(s)</span>
              <span className="mono" style={{ fontSize: 15, fontWeight: 700, color: T.warning }}>{(stats.pending || 0).toLocaleString('fr')} €</span>
            </div>
          )}
        </Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <Card T={T}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 14, color: T.text }}>Actions rapides</div>
          ...
          {stats?.pendingCount > 0 && (
            <Card T={T} style={{ background: T.warningBg, borderColor: `${T.warning}30` }}>
              <div style={{ fontSize: 12, color: T.warning, fontWeight: 700, marginBottom: 6 }}>⚠️ {stats.pendingCount} facture(s) en retard</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{(stats.pending || 0).toLocaleString('fr')} € en attente</div>
            </Card>
          )}
        </div>
...
const PaidWork = ({ T }) => {
  ...
  const openClient = async (c) => {
    ...
    try {
      ...
      // Charger les factures payées avec leurs items
      ...
      // Charger les daily_services inactifs (payés)
...
const Invoices = ({ T, reloadTimerSessions }) => {
  ...
  const loadClientData = useCallback(async (clientId) => {
    ...
    try {
      ...
      // Charger toutes les factures du client pour récupérer les session_ids déjà utilisés
      ...
      // Extraire tous les session_ids déjà dans une facture
      ...
      // Garder uniquement celles qui ne sont dans aucune facture
      const availableSessions = (allSessions || []).filter((s) => !usedSessionIds.has(s.id))
  ...
  const create = async () => {
    ...
    try {
      ...
      if (invoiceMode === 'hours' && sessionIdsToMark.length > 0) {
        ...
        if (updateRes.error) console.error('Erreur update sessions (create):', updateRes.error)
      }
    ...
    try {
      ...
      if (status === 'payée') {
        ...
        if (invoice?.items && Array.isArray(invoice.items)) {
          // 1. Marquer les sessions invoiced = true
          ...
          if (sessionIds.length > 0) {
            const { data: { user } } = await supabase.auth.getUser()
            const { data: updatedSes, error: sesErr, count } = await supabase
              .from('sessions')
              .update({ invoiced: true }, { count: 'exact' })
              .eq('user_id', user.id)
              .in('id', sessionIds)
              .select()
          ...
          // 2. Marquer les daily_services inactifs
          ...
          // 3. SEULEMENT APRÈS tous les updates BDD → recharger l'affichage
          await Promise.all([
            invoice.client_id ? loadClientData(invoice.client_id) : Promise.resolve(),
            reloadTimerSessions ? reloadTimerSessions() : Promise.resolve(),
          ])
        } else {
...
export default function App() {
  ...
  const reloadTimerSessions = useCallback(async () => {
    try {
      ...
      // Récupérer tous les session_ids déjà dans une facture
      ...
      // Garder uniquement celles pas encore dans une facture
      const available = (allSessions || []).filter((s) => !usedSessionIds.has(s.id)).slice(0, 20)
      setTimerSessions(available)
    } catch (error) { console.error('Error reloading timer sessions:', error) }
  }, [])
