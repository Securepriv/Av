...
const PaidWork = ({ T }) => {
  ...
  const openClient = async (c) => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser()
      const [sesRes, dsRes, invRes] = await Promise.all([
        supabase.from('sessions').select('*').eq('client_id', c.id).eq('invoiced', true).order('date', { ascending: false }),
        supabase.from('daily_services').select('*').eq('user_id', user.id).eq('client_id', c.id).eq('is_active', false).order('date_added', { ascending: false }),
      ...
      setPaidDailyServices(dsRes.data || [])
  ...
  const changeStatus = async (id, status) => {
    try {
      ...
      if (status === 'payée') {
        ...
        if (invoice?.items) {
          // Marquer les sessions comme facturées/payées
          ...
          // Marquer les daily_services inclus comme inactifs (is_active = false) pour qu'ils quittent l'historique
          const dsServices = (invoice.items || []).filter((item) => item.unit === 'unité' && !item.session_id).map((item) => item.service)
...
export default function App() {
  ...
  const reloadTimerSessions = useCallback(() => {
    supabase.from('sessions').select('*,clients(name,color)').eq('invoiced', false).order('date', { ascending: false }).limit(20).then((res) => setTimerSessions(res.data || []))
  }, [])
