...
const Invoices = () => {
  ...
  const [toast, setToast] = useState(null)
  const [form, setForm] = useState({clientId:'',items:[{service:'',qty:1,rate:0}],dueDate:''})
  ...
  const total = form.items.reduce((a,i)=>a+Number(i.qty)*Number(i.rate),0)
  ...
  const create = async () => {
    ...
    const {error} = await supabase.from('invoices').insert({
      ...
      invoice_number:num, items:form.items,
      amount:total, due_date:form.dueDate||null, status:'brouillon'
    })
    if(error) notify(error.message,'error')
    else { notify('Facture créée !'); setShowForm(false); setForm(f=>({...f,items:[{service:'',qty:1,rate:0}],dueDate:''})); load() }
    setSaving(false)
  }
      ...
      {showForm&&(
        <Card glow style={{marginBottom:20}} className="fade-up">
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:16}}>
            <div>
              <label style={{fontSize:12,color:T.textMuted,fontWeight:500,display:'block',marginBottom:6}}>Client *</label>
              <select value={form.clientId} onChange={e=>setForm({...form,clientId:e.target.value})} style={{width:'100%',background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'10px 14px',color:T.text,fontSize:13}}>
                {clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
          ...
          <div style={{marginBottom:14}}>
            <div style={{fontSize:12,color:T.textMuted,fontWeight:500,marginBottom:10}}>Lignes de prestation</div>
            {form.items.map((item,i)=>(
              <div key={i} style={{display:'grid',gridTemplateColumns:'2fr 70px 110px 36px',gap:8,marginBottom:8,alignItems:'center'}}>
                <select value={item.service} onChange={e=>updateItem(i,'service',e.target.value)} style={{background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'9px 12px',color:item.service?T.text:T.textDim,fontSize:13}}>
                  <option value="">Choisir service...</option>
                  {VA_SERVICES.map(s=><option key={s}>{s}</option>)}
                </select>
                <input type="number" placeholder="Qté" value={item.qty} onChange={e=>updateItem(i,'qty',e.target.value)} style={{background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'9px 10px',color:T.text,fontSize:13,textAlign:'center'}}/>
                <input type="number" placeholder="Tarif €" value={item.rate||''} onChange={e=>updateItem(i,'rate',e.target.value)} style={{background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,padding:'9px 12px',color:T.text,fontSize:13}}/>
                <button onClick={()=>setForm(f=>({...f,items:f.items.filter((_,j)=>j!==i)}))} style={{background:T.surfaceHigh,border:`1px solid ${T.border}`,borderRadius:8,color:T.danger,cursor:'pointer',padding:'9px',fontSize:12}}>✕</button>
              </div>
            ))}
            <Btn small variant="subtle" onClick={addItem}>+ Ligne</Btn>
          </div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',borderTop:`1px solid ${T.border}`,paddingTop:16}}>
            <div className="mono" style={{fontSize:20,fontWeight:500}}>Total : <span style={{color:T.accent}}>{total.toLocaleString('fr')} €</span></div>
            <div style={{display:'flex',gap:10}}>
              <Btn onClick={create} loading={saving} disabled={!form.clientId||total===0}>Créer</Btn>
              <Btn variant="ghost" onClick={()=>setShowForm(false)}>Annuler</Btn>
            </div>
          </div>
        </Card>
